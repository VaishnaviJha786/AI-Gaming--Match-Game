...
headers_vision = {'Ocp-Apim-Subscription-Key': 'd58628fefa414accb86a2a885c73432e'}
...
def calculate_move(gamestate):
    ...
    print(" Bonus: {}".format(gamestate["Bonus"]))
    print(" Multiplier: {}".format(gamestate["Multiplier"]))
    ...
    # If we don't have any matching tiles
        ...
        match_pairs = search_for_matching_tiles()
        ...
        # If there are some tiles that we haven't analysed yet
            if match_pairs[0]:
                ...
                for match in match_pairs:
                    print("  New tiles Pair: {}".format(match) + " " + analysed_tiles[match[0]]["Category"] + " " + gamestate["Bonus"])
                    if(analysed_tiles[match[0]]["Category"] == gamestate["Bonus"]):
                        move = match
                        print("  Matching Move with Bonus: {}".format(match))
                        flag = 1
                        break
                    
                print("Move Chosen: {}".format(match))
...
def check_for_landmark(msapi_response): # NOTE: you don't need a landmark_list
    ...
    # Record the name of the landmark that is the subject of the tile
            subject = category["detail"]["landmarks"][0]["name"]
    ...
    for index_1, tile_1 in enumerate(analysed_tiles):
        ...
        for index_2, tile_2 in enumerate(analysed_tiles):
            ...
            if tile_1["State"] == tile_2["State"] == "ANALYSED" and tile_1["Subject"] == tile_2["Subject"] and tile_1["Subject"] is not None and index_1 != index_2:
                ...
                matched_tiles_pair.append([index_1, index_2])
    # If we have not matched any tiles, return no matched tiles
    return matched_tiles_pair
